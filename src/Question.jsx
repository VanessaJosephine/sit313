...
const Question = () => {
    ...
    const handleThumbsUp = async (e) => {
        ...
        const num = (await getDoc(doc(db, 'questions', question.id, 'comments', e.target.id))).get('thumbsup') + 1
        await updateDoc(doc(db, 'questions', question.id, 'comments', e.target.id), {
            "thumbsup": num,
        });
    }
    const handleThumbsDown = async (e) => {
        console.log('You clicked: ' + e.target.id);
        const num = (await getDoc(doc(db, 'questions', question.id, 'comments', e.target.id))).get('thumbsup')
        if (num != 0) {
            await updateDoc(doc(db, 'questions', question.id, 'comments', e.target.id), {
                "thumbsup": num - 1,
            });
    ...
    //    }
    //    if (text2 !== null) {
    //        await addDoc(collection(db, 'questions', question.id, 'comments', e.target.id, 'replies'), {
    //            text: text2,
    //            date: new Date().getTime(),
    //            author: currentUser.email,
    //            thumbsup: 0,
